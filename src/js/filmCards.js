...
function getGenreNames(genreIds) {
  ...
  function getReleaseYear(response) {
    if (response.media_type === 'movie') {
      return response.release_date ? response.release_date.split('-')[0] : 'Unknown Year';
    }
    if (response.media_type === 'tv') {
      return response.first_air_date ? response.first_air_date.split('-')[0] : 'Unknown Year';
    }
    return 'Unknown Year';
  }
  ...
  export async function createFilmCard(dataPromise) {
    ...
    data.results.forEach(response => {
      ...
      const rating = typeof response.vote_average === 'number' ? response.vote_average.toFixed(2) : 'N/A';
