...
export const openFilmModal = (filmData, cardHtml) => {
  if (!filmData || typeof filmData !== 'object' || !filmData.genre_ids) {
    console.error('Datele filmului nu sunt complete sau sunt într-un format incorect.');
    return;
  }
  ...
  if (filmData.media_type) {
    originalTitle = filmData.media_type === 'movie'
  ...
  const modalHtml = `
    <div class="film-modal">
        ...
        <p class="film-modal-score"><span>Score: ${typeof filmData.vote_average === 'number' ? filmData.vote_average.toFixed(2) : 'N/A'}</span></p>
        ...
        <div class="film-modal-actions">
          ...
          <button id="addToQueueBtn">${isFilmInQueue ? 'REMOVE FROM QUEUE' : 'ADD TO QUEUE'}</button>
        </div>
      </div>
    </div>`;
  ...
  const setButtonStates = () => {
    ...
    if (filmObject) {
      addToQueueBtn.textContent = filmObject.isInQueue ? 'REMOVE FROM QUEUE' : 'ADD TO QUEUE';
      addToWatchedBtn.textContent = filmObject.isWatched ? 'REMOVE FROM WATCHED' : 'ADD TO WATCHED';
    }
  };
  setButtonStates();
  ...
  addToQueueBtn.addEventListener('click', () => {
    const filmObject = getFromStorage(filmData.id);
    if (filmObject) {
      updateFilmInStorage(filmData.id, !filmObject.isInQueue, filmObject.isWatched);
...
function removeFilmCardFromDOM(filmId) {
  ...
  const filmCard = document.querySelector(`.movie-wrapper__card[data-filmid="${filmId}"]`);
  if (filmCard) {
    filmCard.remove();
